Shader "Custom/UV"
{
	...
	{
		return saturate(abs(frac(offset + t) * 5 - 1) + yOffset);
	}
	...
	fixed4 frag(v2f i) : SV_Target
	{
		...
		if (diff > 0)
			intersect = 1 - smoothstep(0, _ProjectionParams.w * 5, diff);

		float rim = 1 - abs(dot(i.normal, normalize(i.viewDir))) * 2;
		float northPole = (i.objectPos.y - 0.45);
		float glow = max(max(intersect, rim), 0);

		fixed4 glowColor = fixed4(lerp(_Color.rgb, fixed3(1, 0.2, 0.8), pow(glow, 1)), 1);
